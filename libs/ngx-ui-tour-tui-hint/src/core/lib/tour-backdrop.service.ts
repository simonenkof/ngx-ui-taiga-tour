...
export class TourBackdropService {
  ...
  private removeBackdropElement() {
    this.backdropElements.forEach((backdropElement) =>
      this.renderer.removeChild(this.parentContainer, backdropElement),
    );
  ...
  private createBackdropElements() {
    return Array.from({ length: 4 }).map(() => this.createBackdropElement());
  }
